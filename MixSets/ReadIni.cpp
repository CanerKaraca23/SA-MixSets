...
void MixSets::ReadIni_BeforeFirstFrame()
{
	...
	{
		if (lang == languages::PT)
			lg << "\nERROR: O execut�vel do seu jogo n�o � compat�vel. Use Crack 1.0 US Hoodlum ou Compact.\n";
		else
	...
	if (!bReloading && ReadIniFloat(ini, &lg, "Graphics", "FxDistanceMult", &f)) {
		if (ReadMemory<uintptr_t>(0x4AB032, true) == 0x00859AA0) {
			...
			if (lang == languages::PT)
				lg << "\nAVISO: 'FxDistanceMult' n�o foi ativada pois outro mod alterou o valor.\n";
			else
...
void MixSets::ReadIni()
{
	...
	if (!bReloading && ReadIniBool(ini, &lg, "Graphics", "Fix2DGunflash")) {
		if (bGunFuncs) {
			if (lang == languages::PT)
			{
				lg << "Fix2DGunflash desativado pois voc� j� est� usando a corre��o de efeito de tiro do GunFuncs." << "\n";
			}
			else {
			...
			if (lang == languages::PT)
			{
				lg << "Fix2DGunflash desativado pois voc� j� est� usando a corre��o de efeito de tiro do IMFX." << "\n";
			}
			else {
